...
public class CommitteePage extends JFrame implements ActionListener {
	...
	public CommitteePage() {
		...
		eventTable.setModel(new DefaultTableModel(
				new Object[][] 
						...
						new String[] 
								{
						"Type", "location", "description", "Date"
								}
				));
	...
	public void actionPerformed(ActionEvent e){
		...
		if(action.equals ("Add Event")){
			String date, eventType, location, info;

			date = dateAndTimeField.getText();
			eventType = typeField.getText();
	...
	public void displayEventList()
	{
		...
		if(eventList.size() != 0){
			for(int i = 0; i < eventList.size(); i++){
				list.add(new Object[] {
						eventList.get(i).getEventType(),
						eventList.get(i).getLocation(),
						eventList.get(i).getDate(),
						eventList.get(i).getInfo()
				});
			}
		}

		eventTable.setModel(new DefaultTableModel(list.toArray(new Object[][]{}),new String[] {"Event Type", "Location", "Date", "Info"}));
	}
